...
mongoose.connect("mongodb://localhost/viewsdb_v6");
...
app.get("/views", function(req, res){
    ...
    View.find({}, function(err, allViews){
       if(err) {
           ...
           res.render("views/index", {views: allViews, currentUser: req.user});
       }
    });
});
...
app.get("/views/:id/comments/new", isLoggedIn, function(req, res){
...
app.post("/views/:id/comments", isLoggedIn, function(req, res){
    //lookup view by id
    View.findById(req.params.id, function(err, view){
       if(err) {
           console.log(err);
...
app.post("/register", function(req, res){ //register method takes a user object that doesn't have a password attached, and then a hashed the password in the next method
